...
function loadConfiguration(newEnvironment, options) {
  ...
  if (newEnvironment && !options.force) {
    ...
    if (newEnvironment) {
      console.log("###########setting here!!!f")
      process.env.NODE_ENV = newEnvironment;
    } else {
      console.log("###########setting heref", typeof process.env.NODE_ENV)
    ...
    if (options.baseConfiguration || options.appConfiguration) {
      ...
      return setConfiguration(process.env.NODE_ENV, options).then(function (
        result) {
        whenReady.resolve(result);
        return result;
      });
    } else {
      return loadConfig(process.env.NODE_ENV, options).then(function (result) {
        whenReady.resolve(result);
        return result;
      });
    }
...
exports.getConfig = function (key, ignoreCache) {
  ...
  try {
    ...
    if (!result) {
      ...
      // app init.
      var path = process.cwd() + '/config/' + process.env.NODE_ENV + '/' +
        key +
        '.json';
