...
function getConnection() {
  if (connection) {
    ...
    return dbUtils.createConfiguredConnections(null, null, config)
      .then(function () {
...
function dispatch(configOptions) {
  ...
  if (command === 'help' && _.isString(subCommand) && _.isFunction(handlers[
      subCommand][2])) {
    var helpHandler = handlers[subCommand][2];
    helpHandler();
  } else {
