...
function makeResultHandler(request, response, options, mapperConfig) {
  ...
  function writeErr(err, code) {
    ...
    if (mapperConfig[code.toString()] && mapperConfig[code.toString()].sendValue) {
...
function execQueryDecorators(decorators) {
  var decoType = typeof(decorators);
  var decorators = (decoType === 'function' || isArray(decoType)) ?
    decorators : function() {};
