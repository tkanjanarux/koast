...
describe('Error handling', function() {
  ...
  it('Should return ValidationError when missing props', function(done) {
    var cp = dbUtils.createSingleConnection('testdb', dbConfig, schemas);
    cp.then(function(connection) {
      var mapper = mongoMapper.makeMapper(connection);
      expect(mapper).to.not.be.undefined;
      expect(mapper).to.have.property('get');

      var poster = mapper.post({ model: 'testcollection' });
  ...
  // TODO
  // Make db-utils not depend on prior tests. There's an issue
  // where dbUtils.createSingleConnection fails because "the handle
  // already exists".
  xit('Should not return ValidationError text when missing prop if config says so',
  function(done) {
    var cp = dbUtils.createSingleConnection('testdb', dbConfig, schemas);
    cp.then(function(connection) {
      var mapper = mongoMapper.makeMapper(connection);
