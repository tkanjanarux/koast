...
var PASSWORD_FIELD_NAME = 'password';
...
exports.saveUser = function(user, password) {
  ...
  bcrypt.genSalt(SALT_WORK_FACTOR, function(err, salt) {
    ...
    bcrypt.hash(password, salt, function(err, hash) {
      ...
      user[PASSWORD_FIELD_NAME] = hash;

      // Save the user
      user.save(function(err, user) {
        if (user) {
          var sanitizedUser = R.omit(
            [PASSWORD_FIELD_NAME],
            user.toObject? user.toObject() : user);

          deferred.resolve(sanitizedUser);
        } else if (err) {
          return deferred.reject(err);
        }
      });
    ...
    users.findOne(userQuery).exec()
      .then(function(user) {
        ...
        if (!user) {
          ...
          return comparePasswords(password, user[PASSWORD_FIELD_NAME])
...
exports.setup = function(app, users, config) {
  ...
  app.post('/auth/login', function(req, res, next) {
    passport.authenticate('local', function(err, user, info) {
      ...
      var sanitizedUser = R.omit([PASSWORD_FIELD_NAME], user);
      config.callback(sanitizedUser, req, res, next);
    })(req, res, next);
  });
